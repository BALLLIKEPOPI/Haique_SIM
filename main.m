...
rhs = [(t_theta - (I3-I2)*phi*psi - 3)/I1; ...
        (t_phi - (I1-I3)*theta*psi - 3)/I2; ...
        (t_psi - (I2-I1)*theta*phi - 3)/I3];
...
x01 = [0; 0; 0; 0; 0];
xs = [0 ; pi/8 ; 0]; % Reference posture. % 这里要改
xs1 = [0; 0; 0; 0; 0];
x_dot = [0; 0; 0];
x_doth(:,1) = x_dot;
obj_h = [];
...
%     xs = control_sliders();
    ...
    u_cl= [u_cl ; u(1,:)];
%     t(mpciter+1) = t0;
    rou*c1*abs(x_dot(2))*x0(2)
    % Apply the control and shift the solution
    [t0, x0, u0] = shift(h, t0, x0, u, f, x_dot);
    xx(:,mpciter+2) = x0;
%     x0 = x0 + unifrnd(-pi/5,pi/5,3,1);
