...
% 
% rhs = [(t_theta - (I3-I2)*phi*psi - rou*c1*abs_theta_dot*theta)/I1; ...
%         (t_phi - (I1-I3)*theta*psi - rou*c2*abs_phi_dot*phi)/I2; ...
%         (t_psi - (I2-I1)*theta*phi - rou*c3*abs_psi_dot*psi)/I3];
% 
% % rhs = [(t_theta - (I3-I2)*phi*psi)/I1; ...
% %         (t_phi - (I1-I3)*theta*psi)/I2; ...
% %         (t_psi - (I2-I1)*theta*phi)/I3];
% 
% f = Function('f', {states, controls, abs_states_dot}, {rhs}); % nonlinear mapping function f(x,u)
% % f = Function('f', {states, controls}, {rhs}); % nonlinear mapping function f(x,u)
% 
% U = SX.sym('U', n_controls, N); % Decision variables (controls)
% P = SX.sym('P', 3*n_states); 
...
% 
% Q = zeros(3,3); Q(1,1) = 10; Q(2,2) = 10; Q(3,3) = 10; % weighing matrices (states)
...
% 
% args.lbx(1:3:3*(N+1),1) = -pi/2; %state theta lower bound
% args.ubx(1:3:3*(N+1),1) = pi/2; %state theta upper bound
% args.lbx(2:3:3*(N+1),1) = -pi/2; %state phi lower bound
% args.ubx(2:3:3*(N+1),1) = pi/2; %state phi upper bound
% args.lbx(3:3:3*(N+1),1) = -pi/2; %state psi lower bound
% args.ubx(3:3:3*(N+1),1) = pi/2; %state psi upper bound
% 
% args.lbx(3*(N+1)+1:3:3*(N+1)+3*N,1) = -15; %t_theta lower bound
% args.ubx(3*(N+1)+1:3:3*(N+1)+3*N,1) = 15; %t_theta upper bound
% args.lbx(3*(N+1)+2:3:3*(N+1)+3*N,1) = -15; %t_phi lower bound 
% args.ubx(3*(N+1)+2:3:3*(N+1)+3*N,1) = 15; %t_phi upper bound
% args.lbx(3*(N+1)+3:3:3*(N+1)+3*N,1) = -15; %t_psi lower bound 
% args.ubx(3*(N+1)+3:3:3*(N+1)+3*N,1) = 15; %t_psi upper bound
...
rhs1 = [-l*sin(alpha)*(f4+f8) + l*sin(beta)*(f2+f6); ...
        l*(f3+f7-f1-f5); ...
        l*k*(f1-f5+f3-f7) + l*(f4*cos(alpha)+f8*cos(alpha)-f2*cos(beta)-f6*cos(beta)); ...
        f1+f5+f3+f7+f4*sin(alpha)+f8*sin(alpha)+f2*sin(beta)+f6*sin(beta); ...
        f2*cos(beta)+f6*cos(beta) + f8*cos(alpha)+f4*cos(alpha)]; % system r.h.s
...
args1.lbg(1:5*(N+1)+2) = 0;  % -1e-20  % Equality constraints
args1.ubg(1:5*(N+1)+2) = 0;  % 1e-20   % Equality constraints
...
x01 = [0; 0; 0; 0; 0];
xs = [0 ; -pi/3 ; 0]; % Reference posture. % 这里要改
xs1 = [0; 0; 0; 0; 0];
x_dot = [0; 0; 0];
x_doth(:,1) = x_dot;
obj_h = [];
...
figure;
subplot(3,2,1); % 上半部分用于显示状态
h1 = plot(t, xx(1,:), 'b', 'LineWidth', 2);
title('image1');
xlabel('t');
ylabel('theta');

subplot(3,2,2); % 上半部分用于显示状态
h2 = plot(t, xx(2,:), 'b', 'LineWidth', 2);
title('image2');
xlabel('t');
ylabel('phi');

subplot(3,2,3); % 上半部分用于显示状态
...
u = [0; 0; 0];
subplot(3,2,4); % 下半部分用于显示控制输入
h4 = plot(t, u', 'r', 'LineWidth', 2);
title('image4');
xlabel('t');
ylabel('t_x');

subplot(3,2,5); % 下半部分用于显示控制输入
h5 = plot(t, u', 'r', 'LineWidth', 2);
title('image5');
xlabel('t');
ylabel('t_y');

subplot(3,2,6); % 下半部分用于显示控制输入
h6 = plot(t, u', 'r', 'LineWidth', 2);
title('image6');
xlabel('t');
ylabel('t_z');
...
%     xx(:,mpciter+2) = x0;
%     x0 = x0 + unifrnd(-0.5,0.5,3,1);
%     x_dot = (xx(:, mpciter+2) - xx(:, mpciter+1))/h;
...
%     t(mpciter+1) = t0;
    ...
    set(h4, 'XData', t, 'YData', u_cl(:,1)); % 更新控制输入图像
    set(h5, 'XData', t, 'YData', u_cl(:,2)); % 更新控制输入图像
    set(h6, 'XData', t, 'YData', u_cl(:,3)); % 更新控制输入图像
